...
def check_clinic_availability(target_date: str) -> Tuple[bool, str]:
    with sync_playwright() as p:
        ...
        try:
            ...
            # ステップ1: 「再診(婦人科)」ボタンをクリック
            _log("2. 『再診(婦人科)』ボタンを探しています...")
            reishin_btn = page.get_by_role("button", name=re.compile(r"再診.*婦人科"))
            if reishin_btn.count() == 0:
                reishin_btn = page.locator("a, input, button").filter(has_text=re.compile(r"再診.*婦人科")).first
            
            reishin_btn.click()
            page.wait_for_timeout(2000)

            # ステップ2: 遷移先で「次へ」ボタンをクリック
            _log("3. 『次へ』ボタンを探しています...")
